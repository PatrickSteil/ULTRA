...
public:
  BuildFreeTransferGraph(BasicShell &shell)
      : ParameterizedCommand(
            shell, "buildFreeTransferGraph",
            "Builds the free transfer graph for fully multimodal ULTRA.") {
...
private:
  ...
  inline void run() const noexcept {
    const std::string inputFile = getParameter("Input file");
    const size_t intermediateWitnessLimit =
        getParameter<size_t>("Intermediate witness limit");
    const size_t finalWitnessLimit =
        getParameter<size_t>("Final witness limit");
    const std::string outputFile = getParameter("Output file");
    const size_t numberOfThreads = getNumberOfThreads();
    const size_t pinMultiplier = getParameter<size_t>("Pin multiplier");
...
private:
  ...
  template <int TIME_FACTOR> inline void run() const noexcept {
    ...
    const int intermediateWitnessLimit =
        getParameter<int>("Intermediate witness limit");
...
public:
  ComputeDelayEventToEventShortcuts(BasicShell &shell)
      : ParameterizedCommand(
            shell, "computeDelayEventToEventShortcuts",
            "Computes delay-tolerant event-to-event transfer shortcuts using "
  ...
  virtual void execute() noexcept {
    ...
    const int departureDelayBuffer =
        getParameter<int>("Departure delay buffer");
    ...
    std::cout << "Computing delay-tolerant event-to-event ULTRA shortcuts "
                 "(parallel with "
              << numberOfThreads << " threads)." << std::endl;
    shortcutGraphBuilder.computeShortcuts(
        ThreadPinning(numberOfThreads, pinMultiplier), arrivalDelayBuffer,
        departureDelayBuffer, memoryLimit);
    Graph::move(std::move(shortcutGraphBuilder.getStopEventGraph()),
                data.stopEventGraph);
    ...
    originalDijkstra.run(vertex, targets, [&](const Vertex other) {
      if (targets.contains(other)) {
        if (originalDijkstra.getDistance(other) < shortcutDistance[other]) {
          ...
          if (shortcutDijkstra.getDistance(other) ==
              originalDijkstra.getDistance(other)) {
        ...
        } else if (originalDijkstra.getDistance(other) >
                   shortcutDistance[other]) {
          shortCount++;
          std::cout << "\nShortcut from " << vertex << " to " << other
...
public:
  ...
  virtual void execute() noexcept {
    ...
    for (const Vertex fromEventVertex : ultraTBData.stopEventGraph.vertices()) {
      const StopId fromStop =
          ultraTBData.getStopOfStopEvent(StopEventId(fromEventVertex));
      const int arrivalTime =
          ultraTBData.raptorData.stopEvents[fromEventVertex].arrivalTime;
      for (const Edge edge :
           ultraTBData.stopEventGraph.edgesFrom(fromEventVertex)) {
        const Vertex toEventVertex =
            ultraTBData.stopEventGraph.get(ToVertex, edge);
        const int departureTime =
            ultraTBData.raptorData.stopEvents[toEventVertex].departureTime;
        ...
        const StopId toStop =
            ultraTBData.getStopOfStopEvent(StopEventId(toEventVertex));
        ...
        if (shortcutEdge == noEdge) {
          ...
          const int realTravelTime =
              stopShortcuts.get(TravelTime, shortcutEdge);
