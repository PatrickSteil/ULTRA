...
class ReverseTripBasedData : public ParameterizedCommand {

public:
  ReverseTripBasedData(BasicShell &shell)
      : ParameterizedCommand(
            shell, "reverseTripBasedData",
            "Generates a reversed copy of the given Trip-Based network.") {
    addParameter("Trip-Based binary");
    addParameter("Output file");
  }

  virtual void execute() noexcept {
    const std::string tbFile = getParameter("Trip-Based binary");
    const std::string outputFile = getParameter("Output file");

    TripBased::Data data(tbFile);
    data.printInfo();
    TripBased::Data reverse = data.reverseNetwork();
    reverse.printInfo();
    reverse.serialize(outputFile);
  }
};
...
private:
  template <typename GRAPH>
    requires std::is_rvalue_reference_v<GRAPH &&>
  ...
  template <typename GRAPH, typename NETWORK>
    requires std::is_rvalue_reference_v<GRAPH &&>
...
public:
  ...
  virtual void execute() noexcept {
    ...
    const Geometry::Rectangle boundingBox =
        Geometry::Rectangle::BoundingBox(min, max);
...
public:
  ...
  virtual void execute() noexcept {
    ...
    const std::string outputFile = getParameter("Output file");
    const bool buildTransitiveClosure =
        getParameter<bool>("Build transitive closure?");
