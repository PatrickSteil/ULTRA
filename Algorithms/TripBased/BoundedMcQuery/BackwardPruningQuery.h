...
public:
  BackwardPruningQuery(const Data &data, const TransferGraph &transferGraph,
                       const TransferGraph &reverseTransferGraph,
                       const std::vector<int> &transferFromSource,
                       const std::vector<int> &transferToTarget,
      ...
      : data(data), transferGraph(transferGraph),
        ...
        sourceStop(noStop), targetStop(noStop), sourceDepartureTime(never),
        roundOffset(-1), round(-1), maxTrips(-1),
        properArrivalTimes(data.numberOfStopEvents()), profiler(profiler) {
    for (const Edge edge : data.stopEventGraph.edges()) {
      ...
      edgeLabels[edge].departureStop =
          data.getStopOfStopEvent(departureStopEvent);
    ...
    for (StopEventId stopEvent(0); stopEvent < data.numberOfStopEvents();
         stopEvent++) {
      properArrivalTimes[stopEvent] =
          data.arrivalEvents[stopEvent].arrivalTime -
          data.minTransferTime(data.getStopOfStopEvent(stopEvent));
    }
  }

  inline void run(const StopId source, const int originalDepartureTime,
    ...
    for (const RAPTOR::ArrivalLabel &label :
      ...
      sourceDepartureTime =
          -((label.arrivalTime - originalDepartureTime) * arrivalSlack +
            originalDepartureTime);
      roundOffset = maxTrips -
                    std::min(size_t(std::ceil(label.numberOfTrips * tripSlack)),
                             lastNumberOfTrips - 1);
