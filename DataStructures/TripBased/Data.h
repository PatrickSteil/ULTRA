...
public:
  inline const TransferGraph &getTransferGraph() const noexcept {
    return raptorData.transferGraph;
  }
  ...
  inline int minTransferTime(const StopId stop) const noexcept {
    return raptorData.minTransferTime(stop);
  }
  ...
  inline size_t numberOfTripsInRoute(const RouteId route) const noexcept {
    return raptorData.numberOfTripsInRoute(route);
  }
  ...
  inline StopId getStop(const TripId trip,
    ...
    Assert(isTrip(trip), "The id " << trip << " does not represent a trip!");
    Assert(index < numberOfStopsInTrip(trip),
