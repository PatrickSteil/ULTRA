...
#include "../Container/Distribution.h"
    ...
    for (RouteId i = RouteId(0); i < routes.size(); i++) {
      ...
      for (const Intermediate::Trip &trip : route) {
        for (const Intermediate::StopEvent &stopEvent : trip.stopEvents) {
          ...
          // TODO for testing sigma = 1.0
          data.delayDistribution.emplace_back(
              GaussianDist((double)stopEvent.arrivalTime, (double)1.0),
              GaussianDist((double)stopEvent.departureTime, (double)1.0));
        }
      }
    }
...
public:
  ...
  stopEventsOfTrip(const RouteId route, const size_t tripNum) const noexcept {
    Assert(isRoute(route),
           "The id " << route << " does not represent a route!");
    Assert(tripNum < numberOfTripsInRoute(route),
  ...
  getTripIterator(const RouteId route, const StopIndex stopIndex,
    ...
    Assert(isRoute(route),
           "The id " << route << " does not represent a route!");
    Assert(stopIndex < numberOfStopsInRoute(route),
           "The route " << route << " has only " << numberOfStopsInRoute(route)
                        << " stops, but stop index " << stopIndex
                        << " was requested!");
    Assert(currentTrip >= firstTripOfRoute(route),
           "The specified trip is not part of the route!");
    Assert(currentTrip <= lastTripOfRoute(route),
           "The specified trip is not part of the route!");
    Assert((currentTrip - firstTripOfRoute(route)) %
                   numberOfStopsInRoute(route) ==
               0,
...
public:
  ...
  averageTravelTimeGraph(const int minTime, const int maxTime) const noexcept {
    ...
    for (Edge edge : topology.edges()) {
      ...
      Assert(topology.get(TravelTime, edge) >= 0,
...
public:
  ...
  inline void serialize(const std::string &fileName) const noexcept {
    IO::serialize(fileName, firstRouteSegmentOfStop, firstStopIdOfRoute,
                  ...
                  delayDistribution, stopData, routeData,
                  implicitDepartureBufferTimes, implicitArrivalBufferTimes);
  ...
  inline void writeTripCSV(const std::string &fileName) const noexcept {
    ...
    Assert(file, "cannot open file: " << fileName);
    Assert(file.is_open(), "cannot open file: " << fileName);
    file << "LineId,TripId,StopIndex,ArrivalTime,DepartureTime,MeanArrival,"
            "MeanDeparture,SigmaArrival,SigmaDeparture\n";
    for (const RouteId route : routes()) {
      ...
      for (size_t i = 0; i < numberOfTripsInRoute(route); i++) {
        for (size_t j = 0; j < tripLength; j++) {
          const std::size_t index = (i * tripLength) + j;
          const StopEvent &stopEvent = stopEvents[index];
          file << route.value() << "," << i << "," << j << ","
               << stopEvent.arrivalTime << "," << stopEvent.departureTime << ","
               << (double)delayDistribution[index].first.mean() << ","
               << (double)delayDistribution[index].second.mean() << ","
               << (double)delayDistribution[index].first.stddev() << ","
               << (double)delayDistribution[index].second.stddev() << "\n";
        }
      }
    }
    file.close();
...
private:
  ...
  inline void permutate(const Permutation &fullPermutation,
    ...
    Assert(fullPermutation.size() == transferGraph.numVertices(),
           "Full permutation size ("
               << fullPermutation.size()
               << ") must be the same as number of vertices ("
               << transferGraph.numVertices() << ")!");
    Assert(stopPermutation.size() == numberOfStops(),
           "Stop permutation size ("
               << stopPermutation.size()
               << ") must be the same as number of stops (" << numberOfStops()
               << ")!");
...
public:
  ...
  std::vector<std::pair<GaussianDist, GaussianDist>> delayDistribution;
