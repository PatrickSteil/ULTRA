...
struct WalkingParetoLabel {
  ...
  inline bool operator<(const WalkingParetoLabel &other) const noexcept {
    return std::tie(numberOfTrips, arrivalTime, walkingDistance) <
           std::tie(other.numberOfTrips, other.arrivalTime,
...
struct ProbabilityParetoLabel {
  inline static constexpr int NumberOfCriteria = 3;

  ProbabilityParetoLabel(const int arrivalTime = never,
                         const double probabilityCost = INFTY,
                         const int numberOfTrips = -1)
      : arrivalTime(arrivalTime), probabilityCost(probabilityCost),
        numberOfTrips(numberOfTrips) {}

  template <typename LABEL>
  ProbabilityParetoLabel(const LABEL &label, const size_t numberOfTrips)
      : arrivalTime(label.arrivalTime), probabilityCost(label.probabilityCost),
        numberOfTrips(numberOfTrips) {}
  ...
  inline double probability() const noexcept {
    return std::exp(-probabilityCost);
  }

  inline bool operator<(const ProbabilityParetoLabel &other) const noexcept {
    return std::tie(numberOfTrips, arrivalTime, probabilityCost) <
           std::tie(other.numberOfTrips, other.arrivalTime,
  ...
  inline int getCriterion(const size_t i) const noexcept {
    ...
    if (i == 0) {
      ...
      return static_cast<int>(-probabilityCost);
    }
  }
...
template <size_t NUM_MODES> struct MultimodalParetoLabel {
  ...
  MultimodalParetoLabel(const LABEL &label, const size_t numberOfTrips)
      : arrivalTime(label.arrivalTime), numberOfTrips(numberOfTrips) {
  ...
  inline MultimodalParetoLabel bucketize(const int factor = 1) const noexcept {
    ...
    for (size_t i = 0; i < NumTransferModes; i++) {
      result.transferTime[i] =
          factor * getTransferTimeBucketValue(transferTime[i]);
    }
    return result;
  }
  ...
  inline bool operator<(const MultimodalParetoLabel &other) const noexcept {
    return std::tie(numberOfTrips, arrivalTime, transferTime[0]) <
           std::tie(other.numberOfTrips, other.arrivalTime,
